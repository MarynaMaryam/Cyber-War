...
public class GetServers
{
	public static HashMap<String,Object> Handle(JsonNode InputData)
	{
		...
		try
		{
			PreparedStatement ps = Database.getConnection().prepareStatement("SELECT count(*) FROM servers WHERE type=?");
			...
			if(dataType > 0)
			{
				ps = Database.getConnection().prepareStatement("SELECT * FROM servers WHERE type=?");
				ps.setInt(1, dataType);
				rs = ps.executeQuery();
			}
